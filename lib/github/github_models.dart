...
class PullRequest {
  ...
  factory PullRequest.fromJson(Map<String, dynamic> json) {
    final head = (json['head'] as Map?)?.letMap();
    final user = (json['user'] as Map?)?.letMap();
    return PullRequest(
      number: (json['number'] as num?)?.toInt() ?? 0,
...
/// A check run reported against a commit.
class CheckRun {
  const CheckRun({
    required this.name,
    required this.status,
  ...
  bool get failed => conclusion == WorkflowRunConclusion.failure;

  factory CheckRun.fromJson(Map<String, dynamic> json) => CheckRun(
    name: json['name'] as String? ?? '',
    status: _runStatus(json['status'] as String?),
    conclusion: json['conclusion'] == null
        ? null
        : _runConclusion(json['conclusion'] as String?),
    detailsUrl: json['html_url'] as String? ?? '',
  );
}
...
/// A workflow run that newly failed between two snapshots.
class FailedRunAlert {
  const FailedRunAlert({
    required this.owner,
    required this.name,
    required this.run,
  });
...
/// Pull requests fetched for one pinned repository, with head check runs.
class PinnedRepoPullRequests {
  const PinnedRepoPullRequests({
    required this.owner,
    required this.name,
  ...
  final Map<int, List<CheckRun>> checks;
}

/// Releases fetched for one pinned repository.
class PinnedRepoReleases {
  const PinnedRepoReleases({
    required this.owner,
    required this.name,
